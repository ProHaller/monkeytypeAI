...
class TypingTest:
    def __init__(self, stdscr, text, theme):
        self.stdscr = stdscr
        self.to_type_text = text.strip()
    ...
    def display_wpm(self):
        self.stdscr.addstr(2, 0, " " * 20, curses.color_pair(3))  # Clear the line
        self.stdscr.addstr(2, 0, f"WPM: {self.wpm}", curses.color_pair(3))

    def display_accuracy(self):
        self.stdscr.addstr(3, 0, " " * 20, curses.color_pair(3))  # Clear the line
        self.stdscr.addstr(
            3,
            0,
    ...
    def display_typed_chars(self):
        for i, char in enumerate(self.user_typed_text):
            ...
            self.stdscr.addstr(1, i, char, curses.color_pair(color))

    def display_details(self):
        self.stdscr.addstr(
            0, 0, " " * len(self.theme), curses.color_pair(3)
        )  # Clear the line
        self.stdscr.addstr(0, 0, f"Theme: {self.theme}", curses.color_pair(3))
        self.stdscr.addstr(1, 0, self.to_type_text)
        self.display_wpm()
